...
def aggiungi_quantita(bot, update, user_data, db_manager):
    ...
    db_manager.addProduct(user_data['nome'], user_data['prezzo'], qt)
    user_data.clear()
    return aggiorna_dispensa(bot, update, user_data)
...
def aggiorna_dispensa(bot, update, user_data, db_manager):
...
def aggiornaProdottiButton(bot, update, user_data, chat_data, db_manager):
    ...
    if data.data == 'aggiungi':
        ...
        product_name = db_manager.getProductName_fromId(user_data['product_id'])
        ...
        if db_manager.checkTransaction_forProducts(user_data['product_id']):
            db_manager.removeProduct(user_data['product_id'])
...
def aggiorna_quantita(bot, update, user_data, db_manager):
    products = db_manager.getAllProduct()
    ...
    db_manager.modifyQuantity(user_data['product_id'], currentQt + qtToAdd)
...
def start(bot, update, db_manager):
...
def auth(bot, update, user_data, db_manager):
    if (update.message.text == "Bordello") :
        ...
        if user.first_name and user.last_name:
            u = db_manager.addUser(user.first_name + " " + user.last_name, update.message.chat_id)
        elif user.first_name:
            u = db_manager.addUser(user.first_name + " senza congome" , update.message.chat_id)
        elif user.last_name:
            u = db_manager.addUser("senza nome" + " " + user.last_name, update.message.chat_id)
        elif user.username:
            u = db_manager.addUser(user.username, update.message.chat_id)
        else:
            u = db_manager.addUser("Sconosciuto", update.message.chat_id)
        return ConversationHandler.END
    else:
...
def main():
    ...
    checkActivator = CheckActivatorThread(bot, db_manager)
    checkActivator.start()

    # Inizializzo il thread per il check del backup
    checkBackup = CheckBackupThread(db_manager)
    checkBackup.start()
